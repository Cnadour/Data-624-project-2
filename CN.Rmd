...
mae <- mean(abs(test_predictions - y_test))
r2 <- cor(test_predictions, y_test)^2

cat("KNN RMSE:", rmse, "\n")
cat("KNN MAE:", mae, "\n")
cat("KNN R-squared:", r2, "\n")
...
# Define training control with bootstrapping
train_control <- trainControl(
  method = "boot",    
  number = 50,        
  verboseIter = FALSE # Set TRUE to see training progress
)
...
rf_model <- train(
  ...
  method = "rf",
  trControl = train_control,
  tuneLength = 3  # Or customize hyperparameters via tuneGrid
)

```

```{r}
# If multiple 'mtry' values were tried, you'll see a plot of performance vs. mtry
plot(rf_model)
...
# Evaluate performance (e.g., for regression)
rmse_rf <- sqrt(mean((rf_predictions - y_test)^2))
r2_rf   <- cor(rf_predictions, y_test)^2
mae_rf  <- mean(abs(rf_predictions - y_test))

# Print results
cat("RF RMSE:", rmse_rf, "\n")
cat("RF R-squared:", r2_rf, "\n")
cat("RF MAE:", mae_rf, "\n")



```
## Based on the metrics, the random forest model provides better performance indcated by the lower RMSE, MAE and highter R squared.   
